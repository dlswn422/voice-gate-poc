...
class Intent(str, Enum):
    ...
    [핵심 설계 철학]
    - 1차 모델은 사용자의 발화를 '주제(Level-1 Intent)'로만 분류한다
    - 실행, 제어, 해결 판단은 절대 하지 않는다
    - HOW / WHY / ERROR / ISSUE 같은 세부 원인은 여기서 다루지 않는다
    - 한 문장 응답으로 끝낼지, 추가 대화가 필요한지는 Decision Layer(AppEngine)에서 판단한다
    ...
    - 정보 제공으로 끝낼지 여부는 Decision Layer에서 판단
    """
...
class IntentResult(BaseModel):
    ...
    confidence:
        - 해당 발화를 이 intent로 분류해도 될 명확성
        - 0.0 ~ 1.0 범위
        - 낮을수록 애매하며,
          Decision Layer에서 멀티턴(2차 모델)로 위임하기 적합
    """
